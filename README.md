...
## 웹 스토리지 API
### 쿠키와 웹 스토리지
- 쿠키의 여러 단점으로 웹 스토리지 등장
- 서버에서 사용자 PC로 들어와 스토리지 정보를 읽어가지 못함

### 웹 스토리지의 구성
- 세션 스토리지
  - 웹 브라우저 창을 여는 순간부터 닫을 떄까지를 하나의 세션
  - 세션 동안에만 데이터를 기억
- 로컬 스토리지
  - 세션이 끝나도 계속 데이터 보관
  - 브라우저 닫앗다 열어도 데이터 확인 가능

## 지오로케이션 API
- 웹브라우저나 모바일 장치에서 현재 위치 정보를 파악할 수 있게 해줌

### 위치 정보 서비스 알아보기
- http 프로토콜과 지오로케이션 API
  - 반드시 보안이 갖춰진 https 프로토콜 환경에서만 사용해야 함

### geolocation 객체
- 간단히 위치를 알아내고 움직이는 사용자의 위치 추적 가능
